...
if __name__ == "__main__":
    ...
    my_prompt_template = PromptTemplate(
            template=my_query,
            input_variables=["detailed_info_of_person"],
            partial_variables={"format_instructions": summary_parser.get_format_instructions()}
    )
    #llm = ChatOllama(model="llama3.2")
    llm = ChatOllama(model="mistral")
    chain = my_prompt_template | llm | summary_parser
    ...
    #res = chain.invoke(input={"detailed_info_of_person": information})
    #print(type(res),"res:",res)
    #parsed_output=summary_parser.parse(res.content)
    #print(type(parsed_output),"parsed_output:",summary_parser.parse(res.content))
